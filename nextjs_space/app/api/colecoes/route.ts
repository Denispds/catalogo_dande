...
export async function GET(request: NextRequest) {
  try {
    ...
    const where: any = all ? {} : { ativa: true };
    if (apenasDestaque) where.destaque = true;
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { nome, descricao, cor, imagemCapa, destaque } = body ?? {};
    ...
    const colecao = await prisma.catColecao.create({
      data: { nome, slug, descricao: descricao ?? '', cor: cor ?? '#E91E8C', imagemCapa: imagemCapa ?? null, destaque: destaque ?? false, ativa: true },
    });
